...
function ValueChip({
  ...
  return (
    <div className="panel-flat flex items-center gap-2 px-2.5 py-1.5 min-w-[104px]">
      <Icon size={14} className="text-beige-400 shrink-0" />
      <div className="flex flex-col leading-tight">
        <span className="section-label">{label}</span>
        <span className="text-sm font-mono text-beige-200 tabular-nums">{value}</span>
      </div>
    </div>
  )
}
...
function MeterChip({
  ...
  return (
    <div className="panel-flat flex flex-col gap-1 px-2.5 py-1.5 min-w-[104px]">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          <Icon size={12} className={`shrink-0 ${warn ? 'text-muted-red' : 'text-beige-400'}`} />
          <span className={`section-label truncate ${warn ? '!text-muted-red' : ''}`}>{label}</span>
        </div>
        <span className={`text-xs font-mono shrink-0 tabular-nums ${warn ? 'text-muted-red' : 'text-beige-200'}`}>{value}%</span>
      </div>
      <div className="h-1 rounded-full bg-charcoal-800 overflow-hidden">
        <div
          className={`h-full rounded-full transition-[width] duration-300 ${warn ? 'bg-muted-red' : 'bg-termGreen-500'}`}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="section-label tracking-widest mr-0.5 hidden sm:inline">Resources</span>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="section-label tracking-widest mr-0.5 hidden sm:inline">Case Standing</span>
