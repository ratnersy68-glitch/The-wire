    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={type === 'wiretap' ? 'flex-1 btn-primary px-4 py-2 text-sm' : 'flex-1 btn-secondary px-4 py-2 text-sm'}
        >
          Wiretap {state.wiretapActive && '(Active)'}
        </button>
        <button
          onClick={() => setType('search')}
          className={type === 'search' ? 'flex-1 btn-primary px-4 py-2 text-sm' : 'flex-1 btn-secondary px-4 py-2 text-sm'}
        >
          Search Warrant
        </button>
      </div>
      ...
      {type === 'wiretap' && !state.wiretapActive && (
        <select value={targetSuspectId} onChange={(e) => setTargetSuspectId(e.target.value)} className="field px-3 py-2 text-sm mb-3">
      ...
      {type === 'search' && (
        <select value={targetLocationId} onChange={(e) => setTargetLocationId(e.target.value)} className="field px-3 py-2 text-sm mb-3">
      ...
      <textarea
        ...
        className="field px-3 py-2 text-sm mb-4 h-20"
      />

      <button
        disabled={type === 'wiretap' && state.wiretapActive}
        onClick={submit}
        className="w-full px-4 py-3 btn-primary font-mono text-sm disabled:opacity-40"
      >
        Submit Request
      </button>
      ...
      <div className="flex flex-col gap-2">
        ...
        {state.warrants.slice().reverse().map((w) => (
          <div key={w.id} className="panel p-2 text-xs flex justify-between">
