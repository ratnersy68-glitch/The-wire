...
export function CityMapPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      ...
      <p className="text-sm text-beige-400 mb-4">Click a known location to view details. Undiscovered locations stay dark until surveillance, follows, or financial work turns them up.</p>
        ...
        <div className="border border-charcoal-600 bg-navy-900 rounded p-4">
          {selected ? (
            <>
              ...
              <p className="text-xs text-beige-400 mb-2">{TYPE_LABEL[selected.type]} &middot; {selected.district}</p>
              <p className="text-sm text-beige-300 mb-3">{selected.description}</p>
              <p className="text-xs font-mono text-beige-400 mb-3">Heat Level: {selected.heatLevel}</p>

              {linkedSuspects.length > 0 && (
                <div className="mb-3">
                  <p className="text-xs font-mono text-termGreen-500 mb-1">Known people tied here:</p>
